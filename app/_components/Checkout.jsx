    ...
    <div className="min-h-screen px-4 py-8 bg-gray-100 sm:px-6 lg:px-20">
      ...
      <div className="grid max-w-6xl grid-cols-1 gap-8 mx-auto lg:grid-cols-2">
        ...
        <div className="p-6 bg-white rounded-lg shadow-md">
          <h2 className="mb-6 text-2xl font-bold text-pink-700">Address Details</h2>
          <form className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <input ref={firstNameRef} defaultValue={user?.firstName || ""} type="text" placeholder="First Name" className="w-full p-3 border rounded-md" />
              <input ref={lastNameRef} defaultValue={user?.lastName || ""} type="text" placeholder="Last Name" className="w-full p-3 border rounded-md" />
            </div>
            <div className="p-3 bg-gray-100 rounded-md">India</div>
            <input ref={addressRef} type="text" placeholder="House number and street name" className="w-full p-3 border rounded-md" />
            <input type="text" placeholder="Apartment, suite, unit, etc. (optional)" className="w-full p-3 border rounded-md" />
            <input ref={cityRef} type="text" placeholder="Town / City" className="w-full p-3 border rounded-md" />
            <div className="p-3 bg-gray-100 rounded-md">Uttar Pradesh</div>
            <input ref={pincodeRef} type="text" placeholder="PIN Code" className="w-full p-3 border rounded-md" />
            <input ref={phoneRef} type="text" placeholder="Phone" className="w-full p-3 border rounded-md" />
            <input ref={emailRef} defaultValue={user?.primaryEmailAddress?.emailAddress || ""} type="email" placeholder="Email Address" className="w-full p-3 border rounded-md" />
          </form>
        </div>
        ...
        <div className="p-6 bg-white rounded-lg shadow-md">
              ...
              <>
                ...
                {isCouponValid && (
                  <div className="flex justify-between pb-2 text-sm font-medium text-green-700 border-b">
                    <span>
                      Coupon Applied (<span className="uppercase">{coupon.trim()}</span>)
                    </span>
              ...
              <DialogHeader>
                ...
                <div className="flex flex-col items-start gap-2 mt-4">
                  <div className="flex flex-col w-full gap-2 sm:flex-row sm:items-center">
                    <Input onChange={(e) => setCoupon(e.target.value)} type="text" placeholder="Coupon Code" className="flex-1" />
                    <Button onClick={handleApplyCoupon} type="button">Apply</Button>
                  </div>
                  {isCouponValid === true && (
                    <p className="text-green-600">🎉 Coupon Applied! 10% off.</p>
                  )}
                  {isCouponValid === false && (
          ...
          <div className="mt-6 space-y-4 text-sm text-gray-600">
            <p>
              Your data will be used to process your order, support your experience,
              and for other purposes described in our{" "}
              <a href="#" className="text-pink-600 hover:underline">privacy policy</a>.
            </p>
            <label className="flex items-start gap-2">
              <input type="checkbox" className="w-4 h-4 mt-1" onChange={(e) => setTermsAccepted(e.target.checked)} />
              <span>
                I agree to the{" "}
